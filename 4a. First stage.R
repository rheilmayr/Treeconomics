...
fs_mod_cwb <- partial(fs_mod, outcome = "rwi", water_var = "cwb.an", energy_var = "pet.an", mod_type = "lm")
...
site_df <- site_df %>% 
  mutate(fs_result = map(data, .f = fs_mod_bl),
         ...
         fs_result_cwb = map(data, .f = fs_mod_cwb))
...
## Repeat using results from ppt model
fs_ppt <- site_df %>% 
  select(collection_id, fs_result_ppt) %>% 
  unnest(fs_result_ppt)
fs_ppt <- fs_ppt[which(!(fs_ppt %>% pull(mod) %>% is.na())),]
fs_ppt <- fs_ppt %>% 
  unnest(mod) %>% 
  select(-error)
fs_ppt %>% write_csv(paste0(wdir, '2_output/first_stage/site_pet_ppt_std.csv'))

# ## Repeat using results from cwb model
# fs_cwb <- site_df %>% 
#   select(collection_id, fs_result_cwb) %>% 
