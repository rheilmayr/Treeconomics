...
plot_df <- trim_df %>% filter(cwd.spstd > -2, 
                              cwd.spstd < 3, 
                              pet.spstd > -3,
                              pet.spstd < 2)
adj = 0.1

bin = hex_bin(plot_df$cwd.spstd, plot_df$pet.spstd, var4=plot_df$estimate_pet.an, 
              frequency.to.area=TRUE, na.rm = TRUE, height = 0.75, width = 0.75)
bin <- bin %>% mutate(size = (size + adj) / (max(size) + adj))
hexes = hex_coord_df(x=bin$x, y=bin$y, width=attr(bin,"width"), height=attr(bin,"height"), size=bin$size)
hexes$rightness = rep(bin$col, each=6)
ggplot(hexes, aes(x=x, y=y)) + 
  geom_polygon(aes(fill=rightness, group=id)) +
  scale_fill_continuous_diverging(rev = TRUE, mid = 0) 


bin = hex_bin(plot_df$cwd.spstd, plot_df$pet.spstd, var4=plot_df$estimate_cwd.an, 
              frequency.to.area=TRUE, na.rm = TRUE, width = 0.75, height = 0.75)
bin <- bin %>% mutate(size = (size + adj) / (max(size) + adj))
hexes = hex_coord_df(x=bin$x, y=bin$y, width=attr(bin,"width"), height=attr(bin,"height"), size=bin$size)
hexes$rightness = rep(bin$col, each=6)
ggplot(hexes, aes(x=x, y=y)) + 
  geom_polygon(aes(fill=rightness, group=id)) +
  scale_fill_continuous_diverging(rev = TRUE, mid = 0, limits = c(-0.4,.1))
