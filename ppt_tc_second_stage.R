...
flm_df <- read_csv(paste0(wdir, '2_output/first_stage/site_pet_ppt_tc_std.csv'))
...
flm_df <- flm_df %>% 
  mutate(ppt_errorweights = 1 / (std.error_ppt.an.spstd.tc),
...
ppt_est_bounds = quantile(flm_df$estimate_ppt.an.spstd.tc, c(0.01, 0.99),na.rm=T)
pet_est_bounds = quantile(flm_df$estimate_pet.an.spstd.tc, c(0.01, 0.99),na.rm=T)
ppt_spstd_bounds = quantile(flm_df$ppt.spstd.tc, c(0.01, 0.99), na.rm = T)
pet_spstd_bounds = quantile(flm_df$pet.spstd.tc, c(0.01, 0.99), na.rm = T)
...
flm_df <- flm_df %>%
  mutate(outlier = (estimate_ppt.an.spstd.tc<ppt_est_bounds[1]) |
           (estimate_ppt.an.spstd.tc>ppt_est_bounds[2]) |
           (estimate_pet.an.spstd.tc<pet_est_bounds[1]) |
           (estimate_pet.an.spstd.tc>pet_est_bounds[2]))
...
vg <-variogram(estimate_ppt.an.spstd.tc~1, site_points, cutoff = 1500, width = 10)
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
formula = as.formula("estimate_ppt.an.spstd.tc ~ ppt.spstd.tc + (ppt.spstd.tc^2) + pet.spstd.tc + (pet.spstd.tc^2)")
mod_data <- trim_df
ppt_mod <- feols(formula, data = mod_data, weights = mod_data$ppt_errorweights,
...
marg_fx_df <- function(mod){
  ...
  min <- -2.5
  max <- 2.5
  ppt_pred <- predictions(mod, newdata = datagrid(pet.spstd.tc = 0, ppt.spstd.tc = seq(min,max,inc))) %>% 
    mutate(variation = "ppt")
  pet_pred <- predictions(mod, newdata = datagrid(pet.spstd.tc = seq(min,max,inc), ppt.spstd.tc = 0)) %>% 
