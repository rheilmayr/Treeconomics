...
load_growth_data <- function(data_source, species) {
  ...
  if (data_source == "itrdb") {
    ...
    sp_collections <- site_smry %>% filter(species_id == sp_lower) %>% pull(collection_id)
    ...
    df <- dendro %>%
      ...
      select(collection_id, tree_id, core_id, year, growth, ring_age,
             cwd.an, pet.an, cwd.ave, pet.ave)
    ...
    tc_an <- terraclim_df %>%
      select(-year) %>% rename(year = water_year) %>%
...
df <- df %>%
  mutate(cwd_bin = factor(bin_tokens[as.character(cwd_bin_raw)], levels = bin_tokens))

# Reference (omitted) bin = the token of the bin containing the pooled median CWD
ref_raw <- as.character(cut(median(df$cwd.an), breaks = cwd_breaks,
...
age_term <- if (data_source %in% c("itrdb", "fia")) paste0(" + ns(ring_age, ", ns_df, ")") else ""

fit_binned <- function(data) {
  fmla <- as.formula(paste0(
    "growth ~ i(cwd_bin, ref = '", ref_bin, "') + ns(pet.an, ", ns_df, ")",
    age_term, " | tree_id"))
...
fmla_int <- as.formula(paste0(
  "growth ~ i(cwd_bin, cwd.ave, ref = '", ref_bin, "') + i(cwd_bin, ref = '", ref_bin, "')",
  " + ns(pet.an, ", ns_df, ")", age_term, " | tree_id"))
...
int_pred_raw <- predictions(
  mod_int, vcov = FALSE,
  newdata = datagrid(cwd_bin = levels(df$cwd_bin),
                     cwd.ave = ave_q,
...
int_pred <- tibble(
  ...
  left_join(bin_lookup, by = "cwd_bin") %>%
  mutate(baseline = factor(cwd.ave, levels = ave_q,
...
# Figure -----------------------------------------------------------------
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
terc_cols <- c("Low CWD (wet)" = "#0072B2", "Mid CWD" = "#999999",
               "High CWD (dry)" = "#D55E00")
...
pA <- ggplot(pooled_resp, aes(bin_mid, estimate, ymin = lower, ymax = upper)) +
  ...
  labs(subtitle = "A. Pooled response", x = "Annual CWD (mm)",
...
pB <- ggplot(tercile_resp, aes(bin_mid, estimate, color = group, fill = group,
  ...
  labs(subtitle = "B. By historic-CWD tercile", x = "Annual CWD (mm)",
       y = "RWL effect vs. reference bin (mm)", color = "Baseline CWD", fill = "Baseline CWD") +
  coord_cartesian(xlim = core_range) +   # focus on dense, common-support core
  theme_bw()
...
pC <- ggplot(int_pred, aes(bin_mid, estimate, color = baseline)) +
  ...
  labs(subtitle = "C. Interaction model: predicted at baseline P10/P90",
       x = "Annual CWD (mm)", y = "Predicted RWL (mm)", color = "Baseline CWD") +
  coord_cartesian(xlim = core_range) +   # focus on dense, common-support core
  theme_bw()

binned_fig <- (pA + pB) / pC +
  plot_annotation(
    title = paste0("Non-parametric RWL response to annual CWD (", species, ", ", data_source, ")"),
    subtitle = "Step function of annual CWD, controlling for PET + ring age, with tree fixed effects",
    caption = paste0("Reference bin: ", ref_bin, " mm. Outcome: absolute ring width (RWL, mm). ",
                     "Bin width: ", binwidth, " mm."))
binned_fig
