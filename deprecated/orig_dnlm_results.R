...
an_site_clim <- read_rds(paste0(wdir, "out\\climate\\site_an_clim_orig.gz"))
dendro_df <- dendro_df %>% 
...
ave_site_clim <- an_site_clim %>% 
  group_by(collection_id) %>% 
  summarise(cwd.spstd = mean(cwd.an.spstd),
            pet.spstd = mean(pet.an.spstd))


old_incompletes <- ave_site_clim %>% 
  filter(!complete.cases(ave_site_clim)) %>%  
  pull(collection_id)
old_incompletes %>% write_rds(paste0(wdir, "out/climate/old_incompletes.rds"))
